...
class ResNeXt(nn.Module):
    ...
    """
    def __init__(self, baseWidth, cardinality, layers, num_classes=1000):
        """ Constructor
        Args:
...
#        self.fc5 = nn.Linear(512 * block.expansion,512)
        self.feat_bn = nn.BatchNorm1d(512 * block.expansion)
        self.fc0 = nn.Linear(512 * block.expansion, num_classes, bias= True)
...
def resnext50(baseWidth=4, cardinality=32,num_classes=1000):
    """
    Construct ResNeXt-50.
    """
    model = ResNeXt(baseWidth, cardinality, [3, 4, 6, 3], num_classes=num_classes)
    return model
...
def test():
    net = resnext50()
    net.load_state_dict(torch.load('resnext_pretrain.pth'),False)
